...
void DRAW_FFT::draw(ofFbo& fbo, const AUDIO_FFT& fft)
{
	fbo.begin();
		...
		********************/
		...
		if(int(Gui_Global->DrawFFT_ContentsCombination_id) == CONTENTS_TYPE__N1_A1)			snprintf(buf, std::size(buf), "N1");
		else if(int(Gui_Global->DrawFFT_ContentsCombination_id) == CONTENTS_TYPE__N2_A2)	snprintf(buf, std::size(buf), "N2");
		else if(int(Gui_Global->DrawFFT_ContentsCombination_id) == CONTENTS_TYPE__N1_N2)	snprintf(buf, std::size(buf), "N1");
		else																				snprintf(buf, std::size(buf), "A1"); // CONTENTS_TYPE__A1_A2
		...
		title of R
		********************/
		if(int(Gui_Global->DrawFFT_ContentsCombination_id) == CONTENTS_TYPE__N1_A1)			snprintf(buf, std::size(buf), "A1");
		else if(int(Gui_Global->DrawFFT_ContentsCombination_id) == CONTENTS_TYPE__N2_A2)	snprintf(buf, std::size(buf), "A2");
		else if(int(Gui_Global->DrawFFT_ContentsCombination_id) == CONTENTS_TYPE__N1_N2)	snprintf(buf, std::size(buf), "N2");
		else																				snprintf(buf, std::size(buf), "A2"); // CONTENTS_TYPE__A1_A2
		...
		********************/
		...
		for(int i = 0; i < 6; i++){
			snprintf(buf, std::size(buf), "%5.2f", val);
